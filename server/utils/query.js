...
const sqlContent = fs.readFileSync(path.resolve(__dirname,'..','./sql/zjj_blog.sql'),'utf-8')
let pool
...
init.connect()
init.query('CREATE DATABASE zjj_blog',err=>{
    ...
    if(err){
        console.log('zjj_blog database created already')
    }else{
        console.log('create zjj_blog Database')
